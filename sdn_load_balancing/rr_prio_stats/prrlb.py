...
# Poll every 2s so reweighting happens before most flows are assigned
MONITOR_INTERVAL = 2
...
class LoadBalancer(app_manager.RyuApp):
    ...
    # ------------------------------------------------------------------ #
    #  Background monitor — polls every MONITOR_INTERVAL seconds           #
    # ------------------------------------------------------------------ #
    def _monitor(self):
        while True:
            if self.datapath:
    ...
    def flow_stats_reply_handler(self, ev):
        """
        Count ACTIVE flows per server (not duration).
        Servers with fewer active flows than mean get weight=2.
        Servers with more active flows than mean get weight=1.
        tq formula used as sensitivity band only.
        """
        ...
        # sensitivity: 10% of mean, minimum 1
        ...
        print("\n[MONITOR] Active flows: h4=%d h5=%d h6=%d  mean=%.1f  sens=%.1f  tq=%.1f" % (
            active_flows['10.0.0.4'],
            active_flows['10.0.0.5'],
            active_flows['10.0.0.6'],
            mean_count, sensitivity, tq))
        ...
        for s in SERVERS:
            ...
            if all_similar:
                ...
                w = 2   # lighter → send more new flows here
            else:
                w = 1   # heavier → send fewer new flows here
            self.weighted_pool.extend([s] * w)
            print("[MONITOR] %s active=%d → weight=%d%s" % (
                s['ip'], count, w,
                " (all similar)" if all_similar else ""))

        self.rr = self.rr % len(self.weighted_pool)
        print("[MONITOR] pool size=%d  rr_totals: h4=%d h5=%d h6=%d" % (
    ...
    def switch_features_handler(self, ev):
        ...
        self.flow_durations = {}
        self.rr_count = {'10.0.0.4': 0, '10.0.0.5': 0, '10.0.0.6': 0}
        # Reset weighted pool to equal weights on reconnect
        self.weighted_pool = []
        for s in SERVERS:
            self.weighted_pool.extend([s] * s['weight'])
    ...
    def add_flow(self, datapath, priority, match, actions,
                 idle_timeout=180, hard_timeout=0):
    ...
    def packet_in_handler(self, ev):
        ...
        if ip_pkt.dst != VIRTUAL_IP:
            if in_port in {4, 5, 6}:
                ...
                out = parser.OFPPacketOut(
                    ...
                    in_port=in_port, actions=actions_race, data=msg.data)
                dp.send_msg(out)
            return

        # --- VIP traffic → WRR + priority queue ---
        if tcp_pkt:
            proto    = "TCP"
            ...
            if flow_key in self.conn_map:
                ...
                print("[WRR NRT] h4=%d h5=%d h6=%d pool=%d q%d" % (
                    self.rr_count['10.0.0.4'],
                    self.rr_count['10.0.0.5'],
                    self.rr_count['10.0.0.6'],
                    len(self.weighted_pool), queue_id))

            match = parser.OFPMatch(
                in_port=in_port, eth_type=ETH_TYPE_IP, ip_proto=6,
            ...
            if flow_key in self.conn_map:
                ...
                self.conn_map[flow_key] = server
                self.rr_count[server['ip']] += 1
                print("[WRR RT]  h4=%d h5=%d h6=%d pool=%d q%d" % (
                    self.rr_count['10.0.0.4'],
                    self.rr_count['10.0.0.5'],
                    self.rr_count['10.0.0.6'],
                    len(self.weighted_pool), queue_id))
            ...
            match = parser.OFPMatch(
                in_port=in_port, eth_type=ETH_TYPE_IP, ip_proto=17,
                ipv4_src=ip_pkt.src, ipv4_dst=VIRTUAL_IP,
        ...
        # Reverse flow — port-specific so each flow gets its own entry
        if proto == "UDP":
            match_rev = parser.OFPMatch(
                in_port=server['port'],
                eth_type=ETH_TYPE_IP, ip_proto=17,
    ...
    def reply_arp(self, dp, port, eth, arp_pkt):
        ...
        pkt.add_protocol(ethernet.ethernet(
            ethertype=ETH_TYPE_ARP, dst=eth.src, src=VIRTUAL_MAC))
