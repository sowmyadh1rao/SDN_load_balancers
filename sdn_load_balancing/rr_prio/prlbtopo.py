...
# ─────────────────────────────────────────────────────────────────────────────
N_RT_USERS  = 250   # RT  flows from h1  (UDP:5004)
N_NRT_USERS = 250   # NRT flows total    (TCP:80) — split evenly h2/h3
...
def setup_queues(num_ports=NUM_PORTS):
    """
    Two priority queues per port — matches Joshi & Gupta (2024):
      q0: RT  traffic — full 1Gbps  (UDP 5004/554/5005)
      q1: NRT traffic — 4Mbps limit (TCP 80/443/etc.)
    RT packets always get processed before NRT due to higher bandwidth.
    """
    print("*** Configuring TWO priority queues on all %d switch ports (paper: Joshi & Gupta 2024)" % num_ports)
    ...
    print("*** Priority queues configured on all ports")
