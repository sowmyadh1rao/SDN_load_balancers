...
def start_servers(servers):
    ...
    for s in servers:
        ...
        time.sleep(0.5)
        # CHANGE 1: added log files per server
...
def simulate_traffic(clients, n_rt_users=50, n_nrt_users=50, duration=60):
    print("*** Simulating %d RT + %d NRT users for %ds" % (
          n_rt_users, n_nrt_users, duration))
    n         = len(clients)
    ...
    for i, client in enumerate(clients):
        ...
        if rt > 0:
            client.cmd(
                'iperf -c %s -u -p 5004 -P %d -b 10m -t %d &'
                % (VIRTUAL_IP, rt, duration))
        if nrt > 0:
            client.cmd(
                'iperf -c %s -p 80 -P %d -t %d &'
                % (VIRTUAL_IP, nrt, duration))
...
# ─────────────────────────────────────────────────────────────────────────────
# CHANGE 2: Added _collect_host_stats — was completely missing
# ─────────────────────────────────────────────────────────────────────────────
...
def run_experiment(net, duration=60, interval=5):
    ...
    print("*** Step 1 — Starting traffic")

    h1.cmd(
        'iperf -c %s -u -p 5004 -P 50 -b 100k -t %d -i %d '
        '> /tmp/rt_iperf.txt 2>&1 &'
        % (VIRTUAL_IP, duration, interval)
    )
    print("    RT  traffic  h1 -> VIP:5004  -> /tmp/rt_iperf.txt")

    h2.cmd(
        'iperf -c %s -p 80 -P 25 -t %d -i %d '
        '> /tmp/h2_nrt_iperf.txt 2>&1 &'
        % (VIRTUAL_IP, duration, interval)
    )
    print("    NRT traffic  h2 -> VIP:80    -> /tmp/h2_nrt_iperf.txt")

    h3.cmd(
        'iperf -c %s -p 80 -P 25 -t %d -i %d '
        '> /tmp/h3_nrt_iperf.txt 2>&1 &'
        % (VIRTUAL_IP, duration, interval)
    )
    print("    NRT traffic  h3 -> VIP:80    -> /tmp/h3_nrt_iperf.txt")

    _poll_stats_to_file(duration, interval)
    print("    Queue+flow stats polling started")

    #_collect_host_stats(net, duration, interval)
    ...
    _parse_rt_iperf  ('/tmp/rt_iperf.txt')
    _parse_nrt_iperf ('/tmp/h2_nrt_iperf.txt')
    ...
    print("    host_stats.txt      — per host rx/tx over time")
    ...
    try:
        ...
        # ── Packet Loss — must be computed before response time ─────────────
        ...
        # ── Response Time — computed after total_server_rx and transaction_rate
        num_users     = 100  # h1 -P 300 UDP + h2 -P 200 TCP + h3 -P 200 TCP
...
def _poll_stats_to_file(duration, interval):
    # CHANGE 6: removed duplicate nested _poll_stats_to_file definition
    def _poll():
        ...
        while True:
            ...
            # Queue stats — server ports eth4,5,6
...
def create_topology():
    ...
    for server in servers:
        # add default gateway pointing to switch
